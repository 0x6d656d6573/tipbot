...
class HelpCommand extends Command
{
    ...
    async exec(message)
    {
        const commands = this.client.util.embed()
            ...
            .addField(`${process.env.MESSAGE_PREFIX}send 100 0x89y92...38jhu283h9`, `Send ${process.env.SYMBOL} to an external address`)
            .addField(`${process.env.MESSAGE_PREFIX}sendmax 0x89y92...38jhu283h9`, `Send all of your ${process.env.SYMBOL} to an external address`)
            ...
            .addField(`${process.env.MESSAGE_PREFIX}price`, `Display the current ${process.env.SYMBOL} statistics\nAlias: ${process.env.MESSAGE_PREFIX}stats ${process.env.MESSAGE_PREFIX}statistics`)
        ...
        const info = this.client.util.embed()
            ...
            .setDescription(`In order to prevent spamming there is a cooldown period of ${parseFloat(process.env.COOLDOWN) / 1000} seconds per command per user.`)
