...
exports.recipientAddress = async function (interaction, id, member = null) {
    ...
    if (!to) {
        ...
        member.send(`@${interaction.user.username} tipped you some ${Config.get('token.symbol')}! You don't have a wallet yet, so I have created one for you! Use the \`${Config.get('prefix')}help\` command to find out how to make use of my services.`)
